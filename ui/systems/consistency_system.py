...
class SelectionGroupConsistencyCheckerSystem(System):
    ...
    def execute(self, _: float) -> None:
        selection_groups: dict[str, set[EntityId]] = defaultdict(set)
        for (entity_id, (selection_group,)) in self.world.query(SelectionGroup, all_of=(Selected,)):
            selection_groups[selection_group.group].add(entity_id)

        for group_id, entity_ids in selection_groups.items():
            if len(entity_ids) > 1:
