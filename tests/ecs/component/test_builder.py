...
from ecs_architecture.component.registry import ComponentRegistryProtocol, register_component
...
@register_component('mock_attack')
...
def test_build_mock_attack(registry):
    builder = ComponentBuilder(registry)
    components = builder.build({'mock_attack': {'attack': 5.5}})
    assert len(components) == 1
    assert isinstance(components[0], MockComponentAttack)
    assert components[0].attack == 5.5

def test_build_mock_attack_wrong_parameter(registry):
    builder = ComponentBuilder(registry)
    with pytest.raises(TypeError):
        builder.build({'mock_attack': {'hp': 5.5}})

def test_build_mock_hp(registry):
    builder = ComponentBuilder(registry)
    components = builder.build({'mock_hp': {'hp': 53.52}})
    assert len(components) == 1
    assert isinstance(components[0], MockComponentHp)
    assert components[0].hp == 53.52

def test_build_mock_hp_wrong_parameter(registry):
    builder = ComponentBuilder(registry)
    with pytest.raises(TypeError):
        builder.build({'mock_hp': {'attack': 5.5}})

def test_build_mock_attack_hp(registry):
    builder = ComponentBuilder(registry)
    components = builder.build({'mock_attack': {'attack': 5.5}, 'mock_hp': {'hp': 53.52}})
...
def test_build_mock_hp_attack(registry):
    builder = ComponentBuilder(registry)
    components = builder.build({'mock_hp': {'hp': 53.52}, 'mock_attack': {'attack': 5.5}})
    assert len(components) == 2
    assert isinstance(components[0], MockComponentHp)
    assert components[0].hp == 53.52
...
def test_build_non_existing_component():
    ...
    with pytest.raises(KeyError):
        builder.build({'mock_defense': {'defense': 4.5}})    

def test_build_mock_attack_hp_defense(registry):
    builder = ComponentBuilder(registry)
    with pytest.raises(KeyError):
        builder.build({'mock_attack': {'attack': 5.5}, 'mock_hp': {'hp': 53.52}, 'mock_defense': {'defense': 4.5}})
