...
def test_default_text_input():
    ...
    expected_components = [Enabled, Dirty, Text, TextStyle, TextAlignment, Transform, RenderLayer, Hoverable, Pressable, Focusable, InputValue, InputFilter]
...
def test_full_toggle():
    ...
    expected_components = [Enabled, Dirty, Text, TextStyle, TextAlignment, Transform, RenderLayer, Parent, Background, Frame, Hoverable, Pressable, Focusable, InputValue, InputFilter]
    assert len(text_input_components) == len(expected_components)
    for component in expected_components:
        assert any(isinstance(obj, component) for obj in text_input_components)
