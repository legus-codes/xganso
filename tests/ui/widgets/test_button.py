...
def test_default_button():
    ...
    expected_components = [Enabled, Dirty, Text, TextStyle, TextAlignment, Spacing, Transform, RenderLayer, Hoverable, Pressable, Trigger]
...
def test_full_button():
    ...
    expected_components = [Enabled, Dirty, Text, TextStyle, TextAlignment, Spacing, Transform, RenderLayer, Parent, Background, Frame, Hoverable, Pressable, Trigger]
    assert len(button_components) == len(expected_components)
    for component in expected_components:
        assert any(isinstance(obj, component) for obj in button_components)
