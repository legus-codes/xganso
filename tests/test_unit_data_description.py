...
def test_correct_unit(unit_data):
    UnitDataDescription(**unit_data)

def test_correct_unit_without_passive(unit_data):
    unit_data.pop('passive')
    UnitDataDescription(**unit_data)

@pytest.mark.parametrize('field', ['id', 'name', 'unit_class', 'sprites', 'stats'])
def test_incorrect_unit_without_fields(unit_data, field):
    unit_data.pop(field)
    with pytest.raises(ValidationError):
        UnitDataDescription(**unit_data)

@pytest.mark.parametrize('field', ['character', 'board'])
def test_incorrect_unit_without_sprites_fields(unit_data, field):
    unit_data['sprites'].pop(field)
    with pytest.raises(ValidationError):
        UnitDataDescription(**unit_data)

@pytest.mark.parametrize('field', ['hp', 'attack', 'defense', 'speed', 'attack_range', 'movement_range'])
def test_incorrect_unit_without_stats_fields(unit_data, field):
    unit_data['stats'].pop(field)
    with pytest.raises(ValidationError):
        UnitDataDescription(**unit_data)

@pytest.mark.parametrize('field', ['base', 'growth', 'regen'])
def test_incorrect_unit_without_hp_fields(unit_data, field):
    unit_data['stats']['hp'].pop(field)
    with pytest.raises(ValidationError):
        UnitDataDescription(**unit_data)

@pytest.mark.parametrize('field', ['base', 'growth'])
def test_incorrect_unit_without_attack_fields(unit_data, field):
    unit_data['stats']['attack'].pop(field)
    with pytest.raises(ValidationError):
        UnitDataDescription(**unit_data)

@pytest.mark.parametrize('field', ['base', 'growth'])
def test_incorrect_unit_without_defense_fields(unit_data, field):
    unit_data['stats']['defense'].pop(field)
    with pytest.raises(ValidationError):
        UnitDataDescription(**unit_data)

@pytest.mark.parametrize('field', ['base', 'growth'])
def test_incorrect_unit_without_speed_fields(unit_data, field):
    unit_data['stats']['speed'].pop(field)
    with pytest.raises(ValidationError):
        UnitDataDescription(**unit_data)

@pytest.mark.parametrize('field', ['base'])
def test_incorrect_unit_without_attack_range_fields(unit_data, field):
    unit_data['stats']['attack_range'].pop(field)
    with pytest.raises(ValidationError):
        UnitDataDescription(**unit_data)

@pytest.mark.parametrize('field', ['base'])
def test_incorrect_unit_without_movement_range_fields(unit_data, field):
